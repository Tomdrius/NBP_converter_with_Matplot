...
@click.argument('date_start', default=(datetime.now()-timedelta(days=7)).strftime(DATA_FORMAT))
@click.argument('date_end', default=datetime.now().strftime(DATA_FORMAT))
def main(currency, date_start, date_end):

# codes = check_currency_code_exists()
# @click.command()
# @click.option('--currency', '-c', default=DEFAULT_CURRENCY, prompt='Provide the currency', help='The currency code (3 letters)', type=click.Choice(codes, case_sensitive=False), required=True)
