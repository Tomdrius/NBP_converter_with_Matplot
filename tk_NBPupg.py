...
def result_frame_inti(root):
    result_frame = Frame(root, borderwidth=4)
...
def entry_init(frame, label_text):
    label = Label(frame, text=label_text, width=33, anchor=W)
    label.grid(sticky=W, row=len(frame.winfo_children()), column=0, padx=2, pady=2)
    entry = Entry(frame, width=20)
    entry.grid(sticky=W, row=len(frame.winfo_children()) - 1, column=1, padx=2, pady=2)
    return label, entry

def label_show_init(result_frame, entries):
    currency, date_start, date_end = entries
    label = Label(result_frame, text=f'Currency: {currency.get()}. Start Date: {date_start.get()}. End Date: {date_end.get()}', width=52, anchor=W)
    label.grid(sticky=W, row=6, columnspan=2, padx=2, pady=2)

def button_init(root, result_frame, entries):
    button = Button(root, text="Show", command=lambda: label_show_init(result_frame, entries))
